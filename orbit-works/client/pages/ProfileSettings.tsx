...
export default function ProfileSettings() {
  ...
  const onDisconnectWallet = async () => {
    ...
    try {
      ...
      const { data, error } = await client.from('profiles').update({ wallet: null }).eq('id', userId);
      if (error) alert('Error disconnecting wallet: ' + error.message);
      else alert('Wallet disconnected');
    } catch (e: any) {
      alert('Error disconnecting wallet: ' + (e?.message || String(e)));
    }
  };
          ...
          <CardContent className="p-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="font-medium">{profile?.wallet ?? 'Not connected'}</div>
                <div className="mt-2">
